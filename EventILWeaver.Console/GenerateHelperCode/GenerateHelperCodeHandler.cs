...
namespace EventILWeaver.Console.GenerateHelperCode
{
    public class GenerateHelperCodeHandler : HandlerBase
    {
        ...
        {
            var existingTypesWithAutoGeneratedEvents = options.TargetDllPaths
                .SelectMany(targetDllPath => GetExistingTypesWithAutoGeneratedEvents(targetDllPath, _ilWeavedAutoGeneratedEventAttributeName)
            ).ToList();
